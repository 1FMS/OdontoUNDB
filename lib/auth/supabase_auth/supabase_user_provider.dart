...
class OdontoUNDBSupabaseUser extends BaseAuthUser {
  OdontoUNDBSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> odontoUNDBSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = OdontoUNDBSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
